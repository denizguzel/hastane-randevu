...
public class InspectionPlaceDaoImpl extends BaseDaoImpl<InspectionPlaceModel> {
  ...
  public List<InspectionPlaceModel> getAppointmentHeaderByInspectionPlace(InspectionPlaceModel inspectionPlaceModel, Date startDate, Date endDate) {
    Query query = getEntitymanager().createQuery("SELECT DISTINCT(ip) FROM AppointmentModel ap INNER JOIN ap.inspectionPlace ip WHERE ip = :INSPECTION_PLACE AND ap.appointmentStatus = :APPOINTMENT_STATUS AND ap.appointmentDate BETWEEN :START_DATE AND :END_DATE");
